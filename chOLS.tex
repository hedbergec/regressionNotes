...
\chapter{Ordinary least squares regression}\citep{fox}
...
\[
\frac{\partial S}{\partial \beta_1}=2\beta_0\sum_{i=1}^Nx_i+2\beta_1\sum_{i=1}^Nx_i^2-2\sum_{i=1}^Ny_ix_i
\]
and putting the elements back into the sum
\begin{equation}
...
\begin{equation}
\mbox{ln}\left(L\left(\mu,\sigma^2\vert y_1\ldots y_N\right)\right)=-\frac{N}{2}\mbox{ln}\left(2\pi\sigma^2\right)-\frac{1}{2\sigma^2}\sum_{i=1}^N\left(y_i-\mu\right)^2
\end{equation}
and replace the mean with the regression formula
\begin{equation}
\mbox{ln}\left(L\left(\beta_0,\beta_1,\sigma^2 \vert y_1\ldots y_N\right)\right)=-\frac{N}{2}\mbox{ln}\left(2\pi\sigma^2\right)-\frac{1}{2\sigma^2}\sum_{i=1}^N\left(y_i-\beta_0-\beta_1x_i\right)^2
\end{equation}
...
For comprehensive treatment of ordinary least squares regression, see \citep{fox} and \citep{hill}. For maximum likelihood estimation foundations, consult \citep{eliason1993maximum}. Mathematical approaches can be found in \citep{gill2006essential} and \citep{freedman2009statistical}.
