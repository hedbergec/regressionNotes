...
\begin{equation}
\frac{\partial\mbox{ln}\left(L\left(\theta|\psi,{\bf y}\right)\right)}{\partial\theta}=\frac{\partial}{\partial\theta}\left[\frac{y\theta-b\left(\theta\right)}{a\left(\psi\right)}+c\left(y,\psi\right)
...
For comprehensive treatment of generalized linear models and multinomial outcomes, see \citep{glm} and \citep{agresti}. For discussion of proportional odds regression and ordinal logistic regression, consult \citep{fox}. Additional perspectives on extending logistic regression to multiway categorical outcomes can be found in \citep{cameron2005microeconometrics} and \citep{eliason1993maximum}.\section{A note on interactions}
