...
\begin{equation}
MSW=\frac{\sum_{j=1}^k\sum_{i=1}^{n_j}\left(y_{ij}-\bar{y}_j\right)^2}{N-k}
\end{equation}
The ratio then looks like this
\begin{equation}
F=\frac{MSB}{MSW}
\end{equation}
and we test how much of the $F$-distribution (with $k-1$ and $N-k$ degrees of freedom) is left after this value for our Type I error rate of our test of the the null hypothesis that all group means are the same. In other words, our null hypothesis is that all the group means are the same:
\begin{equation}
...
For comprehensive treatment of chi-square tests and categorical data analysis, see \citep{agresti}. For discussion of association measures and categorical independence testing, consult \citep{rice2007mathematical}. Additional perspectives on multivariate categorical analyses can be found in \citep{cameron2005microeconometrics}.
