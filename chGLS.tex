...
\chapter{Generalized least squares, in general}\citep{hill}
...
\begin{equation}
\mbox{ln}\left(L\left({\bf b,\Sigma_{ee} \vert \bf y}\right)\right)=-\frac{N}{2}\mbox{ln}\left(2\pi\right)-\frac{1}{2}\mbox{ln}\left(\mbox{det}\bf \Sigma_{ee}\right)-\frac{1}{2}{\bf \left(y-Xb\right)'\Sigma_{ee}^{-1}\left(y-Xb\right)}
\end{equation}
...
For generalized least squares, see \citep{hill} and \citep{cameron2005microeconometrics}. See Chapter~\ref{sec:het} on heteroskedasticity for related approaches. For maximum likelihood methods underlying GLS, consult \citep{eliason1993maximum}.
