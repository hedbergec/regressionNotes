...
\end{equation}
This is true because if $p$ is the mean and $x_i$ only takes on values of 0 or 1, then $Np$ is the number of observations in which $x_i=1$ and $N\left(1-p\right)$ is the number of observations in which $x_i=0$. Thus, the variance can be calculated as
\begin{equation}
\sigma^2_p = \frac{\sum_{i=1}^N\left(x_i - p\right)^2}{N} = \frac{Np\left(1-p\right)^2 + N\left(1-p\right)p^2}{N} = \frac{Np\left(1-p\right)}{N} = p\left(1-p\right)
\end{equation}
...
\begin{equation}
SE\left(\bar{y}_A-\bar{y}_B\right)=s_{pooled}\sqrt{\frac{1}{n_A}+\frac{1}{n_B}}
\end{equation}
where
\begin{equation}
